	...
	, _log(Logger::getInstance("HTTPJSONRPC"))
{
	...
	_ambilightAPI = new AmbilightAPI(client, _log, localConnection, this, true);
	connect(_ambilightAPI, &AmbilightAPI::SignalCallbackJsonMessage, this, &WebJsonRpc::handleCallback);
	connect(_ambilightAPI, &AmbilightAPI::SignalPerformClientDisconnection, [&]() { _wrapper->closeConnection(); _stopHandle = true; });
...
void WebJsonRpc::handleMessage(QtHttpRequest* request, QString query)
{
	...
	if (!_stopHandle)
	{
		...
		if (nextQueryIndex < 0)
		{
			...
			_unlocked = true;
			_ambilightAPI->handleMessage(data, header);
		}
		else
		{
			const QByteArray& header = request->getHeader("Authorization");
			...
			do
			{
				...
				_ambilightAPI->handleMessage(leftQuery.toUtf8(), header);

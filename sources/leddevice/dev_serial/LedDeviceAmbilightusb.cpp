#include "LedDeviceAmbilightusb.h"

#include <QtEndian>

#include <cassert>

LedDeviceAmbilightusb::LedDeviceAmbilightusb(const QJsonObject& deviceConfig)
...
LedDevice* LedDeviceAmbilightusb::construct(const QJsonObject& deviceConfig)
{
	return new LedDeviceAmbilightusb(deviceConfig);
}

bool LedDeviceAmbilightusb::init(const QJsonObject& deviceConfig)
{
	bool isInitOK = false;
...
void LedDeviceAmbilightusb::CreateHeader()
{
	// create ledBuffer
...
int LedDeviceAmbilightusb::write(const std::vector<ColorRgb>& ledValues)
{
...
void LedDeviceAmbilightusb::whiteChannelExtension(uint8_t*& writer)
{

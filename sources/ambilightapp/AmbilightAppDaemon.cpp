	...
	, _disableOnStart(false)
{
	...
	connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetAccessManager, this, &AmbilightAppDaemon::getAccessManager, Qt::DirectConnection);
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _accessManager.get(), &AccessManager::handleSettingsUpdate);
	...
	connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetPerformanceCounters, this, &AmbilightAppDaemon::getPerformanceCounters, Qt::DirectConnection);
	...
	// Discovery Wrapper
		...
		connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetDiscoveryWrapper, this, &AmbilightAppDaemon::getDiscoveryWrapper, Qt::DirectConnection);
	#endif
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _netOrigin.get(), &NetOrigin::settingsChangedHandler);
	...
	connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetSoundCapture, this, &AmbilightAppDaemon::getSoundCapture, Qt::DirectConnection);
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _soundCapture.get(), &SoundGrabber::settingsChangedHandler);
#endif
	...
	// CEC wrapper
		...
		connect(_wrapperCEC.get(), &WrapperCEC::SignalStateChange, _instanceManager.get(), &AmbilightAppManager::setSignalStateByCEC);
		connect(GlobalSignals::getInstance(), &GlobalSignals::SignalRequestComponent, _wrapperCEC.get(), &WrapperCEC::sourceRequestHandler);
	#endif

	// spawn all Ambilightapp instances (non blocking)
	...
	//Cleaning up Ambilightapp before quit
	connect(parent, &QCoreApplication::aboutToQuit, this, &AmbilightAppDaemon::freeObjects);

	// pipe settings changes and component state changes from AmbilightAPPIManager to Daemon
...
AmbilightAppDaemon::~AmbilightAppDaemon() = default;

void AmbilightAppDaemon::instanceStateChangedHandler(InstanceState state, quint8 instance, const QString& name)
{
...
QJsonDocument AmbilightAppDaemon::getSetting(settings::type type) const
{
...
void AmbilightAppDaemon::freeObjects()
{
	Info(_log, "Cleaning up AmbilightApp before quit [preparing]");
...
void AmbilightAppDaemon::startNetworkServices()
{
	//////////////////////////////////////////
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _jsonServer, &JsonServer::handleSettingsUpdate);
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _webserver, &WebServer::handleSettingsUpdate);
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _sslWebserver, &WebServer::handleSettingsUpdate);
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _ssdp, &SSDPHandler::handleSettingsUpdate);
	...
	connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, _mqtt, &mqtt::handleSettingsUpdate);
...
quint16 AmbilightAppDaemon::getWebPort()
{
	return (quint16)getSetting(settings::type::WEBSERVER).object()["port"].toInt(8090);
}

template<typename T>
void AmbilightAppDaemon::createVideoGrabberHelper(QJsonDocument config, QString deviceName, QString rootPath)
{
	...
	connect(_videoGrabber.get(), &GrabberHelper::SignalCreateGrabber, _videoGrabber.get(), [this, videoDetection, config, deviceName, rootPath]() {
		...
		connect(videoGrabberInstance, &GrabberWrapper::SignalSetNewComponentStateToAllInstances, _instanceManager.get(), &AmbilightAppManager::SignalSetNewComponentStateToAllInstances);
		connect(videoGrabberInstance, &GrabberWrapper::SignalSaveCalibration, _instanceManager.get(), &AmbilightAppManager::saveCalibration);
		connect(_videoGrabber->thread(), &QThread::finished, videoGrabberInstance, &GrabberWrapper::deleteLater);
		connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, videoGrabberInstance, &GrabberWrapper::handleSettingsUpdate);
		connect(_instanceManager.get(), &AmbilightAppManager::SignalInstancePauseChanged, videoGrabberInstance, &T::SignalInstancePauseChanged);
		#ifdef ENABLE_CEC
	...
	connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetVideoGrabber, this, &AmbilightAppDaemon::getVideoGrabber, Qt::DirectConnection);
	_videoGrabberThread->start();
	_videoGrabber->linker.acquire(1);	
}

void AmbilightAppDaemon::createSoftwareGrabberHelper(QJsonDocument config, QString deviceName, QString rootPath)
{
	...
	connect(_systemGrabber.get(), &GrabberHelper::SignalCreateGrabber, _systemGrabber.get(), [this, config, deviceName, rootPath]() {
		...
		if (softwareGrabberInstance != nullptr)		
		{
			...
			connect(_instanceManager.get(), &AmbilightAppManager::SignalSettingsChanged, softwareGrabberInstance, &SystemWrapper::handleSettingsUpdate);
			softwareGrabberInstance->SystemWrapper::handleSettingsUpdate(settings::type::SYSTEMGRABBER, config);
		}
		_systemGrabber->linker.release(1);
	});

	connect(GlobalSignals::getInstance(), &GlobalSignals::SignalGetSystemGrabber, this, &AmbilightAppDaemon::getSystemGrabber, Qt::DirectConnection);
	_systemGrabberThread->start();
	_systemGrabber->linker.acquire(1);	
}

void AmbilightAppDaemon::getInstanceManager(std::shared_ptr<AmbilightAppManager>& retVal)
{
	retVal = _instanceManager;
};

void AmbilightAppDaemon::getAccessManager(std::shared_ptr<AccessManager>& retVal)
{
	retVal = _accessManager;
};

void AmbilightAppDaemon::getSoundCapture(std::shared_ptr<SoundCapture>& retVal)
{
	retVal = _soundCapture;
}

void AmbilightAppDaemon::getSystemGrabber(std::shared_ptr<GrabberHelper>& systemGrabber)
{
	systemGrabber = _systemGrabber;
}

void AmbilightAppDaemon::getVideoGrabber(std::shared_ptr<GrabberHelper>& videoGrabber)
{
	videoGrabber = _videoGrabber;
}

void AmbilightAppDaemon::getPerformanceCounters(std::shared_ptr<PerformanceCounters>& performanceCounters)
{
	performanceCounters = _performanceCounters;
}

void AmbilightAppDaemon::getDiscoveryWrapper(std::shared_ptr<DiscoveryWrapper>& discoveryWrapper)
{
	discoveryWrapper = _discoveryWrapper;
}

void AmbilightAppDaemon::settingsChangedHandler(settings::type settingsType, const QJsonDocument& config)
{
